...
func TestPrepareResponseStateForNewTurnRequiresAzdState(t *testing.T) {
	...
	for _, background := range []bool{false, true} {
		t.Run(fmt.Sprintf("background=%t", background), func(t *testing.T) {
			t.Parallel()
			action := &InvokeAction{flags: &invokeFlags{background: background}}
...
func TestValidateInvokeOperationFlags(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			flags:   invokeFlags{background: true},
			wantErr: "a message argument or --input-file is required",
		},
		{name: "background accepts input", flags: invokeFlags{background: true, message: "hello"}},
		{
			name:  "background accepts no wait",
			flags: invokeFlags{background: true, noWait: true, message: "hello"},
		},
		{
			name:    "no wait requires background",
			flags:   invokeFlags{noWait: true, message: "hello"},
			wantErr: "--no-wait requires --background",
		},
		{name: "continue accepts empty input", flags: invokeFlags{continueRun: true}},
		{
			name:    "continue rejects message",
			flags:   invokeFlags{continueRun: true, message: "hello"},
			wantErr: "--continue and --cancel do not accept a message or --input-file",
		},
		{
			name:    "continue rejects file",
			flags:   invokeFlags{continueRun: true, inputFile: "request.json"},
			wantErr: "--continue and --cancel do not accept a message or --input-file",
		},
		...
		},
		{
			...
			flags:   invokeFlags{continueRun: true, cancel: true},
			wantErr: "--continue and --cancel are mutually exclusive",
		},
		{
			name:    "background and continue are exclusive",
			flags:   invokeFlags{background: true, continueRun: true, message: "hello"},
			wantErr: "--background cannot be combined with --continue or --cancel",
		},
		{
			name:    "background and cancel are exclusive",
			flags:   invokeFlags{background: true, cancel: true, message: "hello"},
			wantErr: "--background cannot be combined with --continue or --cancel",
		},
		{
			name:    "continue rejects session id",
			flags:   invokeFlags{continueRun: true},
		...
		},
		{
			...
			wantErr: "--timeout is not supported with --continue or --cancel",
		},
		{
...
func TestInvokeCommandBackgroundValidation(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want: "background operations are not supported with the invocations protocol",
		},
		{
